...
def generate_launch_description():
    pkg = get_package_share_directory("robotic_arm_manipulation")
    params = os.path.join(pkg, "config", "py_params.yaml")

    return LaunchDescription([
        Node(
            package="robotic_arm_manipulation",
...
To also bring up the existing C++ object detector alongside it, include `robotic_arm_bringup` and add an `object_detector_node` in the same `LaunchDescription`.
