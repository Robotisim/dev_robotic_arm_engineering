...
namespace pick_place {
	...
	namespace {
		...
		std::vector<std::string> buildSourceFrameCandidates(std::string frame_id) {
			std::vector<std::string> candidates;

			auto push_unique = [&](std::string const& frame) {
				if (frame.empty()) {
					return;
				}

				auto it = std::find(candidates.begin(), candidates.end(), frame);
				if (it == candidates.end()) {
					candidates.push_back(frame);
				}
			};

			push_unique(frame_id);
			...
			auto last_slash = frame_id.find_last_of('/');
			if (last_slash != std::string::npos && (last_slash + 1) < frame_id.size()) {
				push_unique(frame_id.substr(last_slash + 1));
			}
	...
	PickPlaceExecutor::PickPlaceExecutor(rclcpp::NodeOptions const& options) : Node("pick_place_executor", options) {
		...
		this->declare_parameter("pick_place_executor.arm_group_name", "panda_arm");
		this->declare_parameter("pick_place_executor.end_effector_link", "panda_hand_tcp");
		...
		this->declare_parameter("pick_place_executor.enforce_min_z", true);
		this->declare_parameter("pick_place_executor.min_allowed_z", 0.0);
		this->declare_parameter("pick_place_executor.use_current_ee_orientation_for_pick", false);
		this->declare_parameter("pick_place_executor.use_fixed_pick_orientation", true);
		this->declare_parameter("pick_place_executor.pick_pose.qx", 1.0);
		this->declare_parameter("pick_place_executor.pick_pose.qy", 0.0);
		this->declare_parameter("pick_place_executor.pick_pose.qz", 0.0);
		this->declare_parameter("pick_place_executor.pick_pose.qw", 0.0);
		...
		this->declare_parameter("pick_place_executor.place_pose.qx", 0.0);
		this->declare_parameter("pick_place_executor.place_pose.qy", 0.0);
		this->declare_parameter("pick_place_executor.place_pose.qz", 0.0);
		this->declare_parameter("pick_place_executor.place_pose.qw", 1.0);
		...
		end_effector_link_ = this->get_parameter("pick_place_executor.end_effector_link").as_string();
		...
		enforce_min_z_ = this->get_parameter("pick_place_executor.enforce_min_z").as_bool();
		min_allowed_z_ = this->get_parameter("pick_place_executor.min_allowed_z").as_double();
		use_current_ee_orientation_for_pick_ =
		    this->get_parameter("pick_place_executor.use_current_ee_orientation_for_pick").as_bool();
		use_fixed_pick_orientation_ = this->get_parameter("pick_place_executor.use_fixed_pick_orientation").as_bool();
		...
		fixed_pick_orientation_.x = this->get_parameter("pick_place_executor.pick_pose.qx").as_double();
		fixed_pick_orientation_.y = this->get_parameter("pick_place_executor.pick_pose.qy").as_double();
		fixed_pick_orientation_.z = this->get_parameter("pick_place_executor.pick_pose.qz").as_double();
		fixed_pick_orientation_.w = this->get_parameter("pick_place_executor.pick_pose.qw").as_double();
		...
		rerun_service_ = this->create_service<std_srvs::srv::Trigger>(
		    "/pick_place/rerun",
		    std::bind(&PickPlaceExecutor::rerunServiceCallback, this, std::placeholders::_1, std::placeholders::_2));

		RCLCPP_INFO(this->get_logger(), "Subscribed to /pick_place/object_pose");
		RCLCPP_INFO(this->get_logger(), "Service ready: /pick_place/rerun");
		...
		RCLCPP_INFO(this->get_logger(), "Arm end-effector link: %s", end_effector_link_.c_str());
		...
		RCLCPP_INFO(this->get_logger(),
		            "Z floor constraint: enforce_min_z=%s, min_allowed_z=%.3f m",
		            enforce_min_z_ ? "true" : "false",
		            min_allowed_z_);
		RCLCPP_INFO(this->get_logger(),
		            "Pick orientation mode: use_current_ee_orientation_for_pick=%s",
		            use_current_ee_orientation_for_pick_ ? "true" : "false");
		RCLCPP_INFO(this->get_logger(),
		            "Pick orientation override: use_fixed_pick_orientation=%s | quat=[%.3f, %.3f, %.3f, %.3f]",
		            use_fixed_pick_orientation_ ? "true" : "false",
		            fixed_pick_orientation_.x,
		            fixed_pick_orientation_.y,
		            fixed_pick_orientation_.z,
		            fixed_pick_orientation_.w);
		RCLCPP_INFO(this->get_logger(),
	...
	void PickPlaceExecutor::objectPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
		...
		auto source_frame_candidates = buildSourceFrameCandidates(msg->header.frame_id);
		for (auto const& candidate_frame : source_frame_candidates) {
			geometry_msgs::msg::PoseStamped pose_for_transform = *msg;
			pose_for_transform.header.frame_id = candidate_frame;

			try {
				transformed_pose = tf_buffer_->transform(pose_for_transform, base_frame_, tf2::durationFromSec(0.1));
				transform_ok = true;
				used_source_frame = candidate_frame;
				break;
		...
		if (!transform_ok) {
			RCLCPP_WARN_THROTTLE(this->get_logger(),
			                     *this->get_clock(),
			                     1000,
			                     "Transform failed (%s -> %s). Checked raw and normalized source frame IDs",
			                     msg->header.frame_id.c_str(),
			                     base_frame_.c_str());
			return;
		}

		if (used_source_frame != msg->header.frame_id) {
			RCLCPP_INFO_THROTTLE(this->get_logger(),
			                     *this->get_clock(),
			                     2000,
			                     "Resolved source frame '%s' to '%s' for TF transform",
			                     msg->header.frame_id.c_str(),
			                     used_source_frame.c_str());
		}
	...
	void PickPlaceExecutor::runPlanningSequence(geometry_msgs::msg::PoseStamped const& object_pose_base) {
		...
		// Ensure target tool Z-axis points downward in base frame (negative Z).
		{
			...
			if (z_axis_target.z() > 0.0) {
				...
				RCLCPP_WARN(this->get_logger(),
				            "Pick orientation had tool Z up (%.3f). Applied 180deg X flip to enforce downward approach.",
				            z_axis_target.z());
			}
		}
		...
		if (enforce_min_z_ && pick_pose.pose.position.z < min_allowed_z_) {
			RCLCPP_ERROR(this->get_logger(),
			             "Pick pose rejected: z=%.3f is below min_allowed_z=%.3f in frame %s",
		...
		// Diagnostic: Check if end-effector is roughly pointing down
		auto current_ee_pose = arm_move_group_->getCurrentPose(end_effector_link_);
		try {
			geometry_msgs::msg::PoseStamped ee_in_base;
			tf_buffer_->transform(current_ee_pose, ee_in_base, base_frame_, tf2::durationFromSec(1.0));

			// Check Z-axis orientation (should point downward, i.e., negative Z component)
			tf2::Quaternion q(ee_in_base.pose.orientation.x,
			                  ee_in_base.pose.orientation.y,
			                  ee_in_base.pose.orientation.z,
			                  ee_in_base.pose.orientation.w);
			tf2::Matrix3x3 m(q);
			tf2::Vector3 z_axis = m.getColumn(2);

			RCLCPP_INFO(
			    this->get_logger(), "Current EE Z-axis direction: [%.3f, %.3f, %.3f]", z_axis.x(), z_axis.y(), z_axis.z());

			if (z_axis.z() > 0.5) {
				RCLCPP_WARN(this->get_logger(),
				            "WARNING: End-effector Z-axis pointing UP (z=%.3f)! This may cause bottom-up approach.",
				            z_axis.z());
			}
		} catch (tf2::TransformException& ex) {
			RCLCPP_ERROR(this->get_logger(), "Transform error during EE orientation check: %s", ex.what());
		}

		geometry_msgs::msg::PoseStamped pre_grasp_pose = computePreGraspPose(pick_pose);
		if (enforce_min_z_ && pre_grasp_pose.pose.position.z < min_allowed_z_) {
			RCLCPP_ERROR(this->get_logger(),
			             "Pre-grasp pose rejected: z=%.3f is below min_allowed_z=%.3f in frame %s",
		...
		// Debug: Log the pre-grasp pose target
		RCLCPP_INFO(this->get_logger(),
		            "Pre-grasp target | xyz: [%.3f, %.3f, %.3f] | quat: [%.3f, %.3f, %.3f, %.3f]",
		...
		if (!planCartesianMove(pick_pose.pose, "cartesian_descent_plan", execute_pick_stages)) {
			setState(State::IDLE);
			return;
		}
	...
	bool PickPlaceExecutor::planToPoseTarget(geometry_msgs::msg::PoseStamped const& target_pose,
		...
		RCLCPP_INFO(this->get_logger(), "[%s] Planning to pose target", stage_name.c_str());
		...
		// Diagnostic pre-check: verify current start state is within joint bounds
		auto current_state = arm_move_group_->getCurrentState(1.0);
		if (!current_state) {
			RCLCPP_WARN(
			    this->get_logger(), "[%s] Unable to fetch current robot state for start-state pre-check", stage_name.c_str());
		} else {
			auto const* joint_model_group = current_state->getJointModelGroup(arm_group_name_);
			if (!joint_model_group) {
				RCLCPP_WARN(this->get_logger(),
				            "[%s] Joint model group '%s' unavailable for pre-check",
				            stage_name.c_str(),
				            arm_group_name_.c_str());
			} else {
				bool start_state_in_bounds = current_state->satisfiesBounds(joint_model_group);
				RCLCPP_INFO(this->get_logger(),
				            "[%s] Start-state bounds check for group '%s': %s",
				            stage_name.c_str(),
				            arm_group_name_.c_str(),
				            start_state_in_bounds ? "valid" : "INVALID");

				std::vector<double> joint_values;
				current_state->copyJointGroupPositions(joint_model_group, joint_values);
				std::string joint_values_str;
				for (size_t i = 0; i < joint_values.size(); ++i) {
					joint_values_str += std::to_string(joint_values[i]);
					if (i + 1 < joint_values.size()) {
						joint_values_str += ", ";
					}
				}
				RCLCPP_INFO(
				    this->get_logger(), "[%s] Start-state joint values: [%s]", stage_name.c_str(), joint_values_str.c_str());
			}
		}
	...
	void PickPlaceExecutor::rerunServiceCallback(const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
		...
		if (!has_stable_detection_ && !sequence_planned_once_) {
			response->success = false;
			response->message = "No stable object pose available yet; wait for detection and retry";
			return;
		}

		if (!has_stable_detection_ && sequence_planned_once_) {
			RCLCPP_WARN(this->get_logger(),
			            "Rerun requested without current stable detection; reusing last planned object pose");
		}

		sequence_planned_once_ = false;
		planning_busy_ = true;
		runPlanningSequence(stable_object_pose_);
		planning_busy_ = false;

		response->success = true;
		response->message = "Rerun request accepted; planning sequence executed";
	}
