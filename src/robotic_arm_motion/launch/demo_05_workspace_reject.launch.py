...
def generate_launch_description() -> LaunchDescription:
    start_panda_arg = DeclareLaunchArgument(
        'start_panda',
        default_value='true',
        description='Start Panda simulation bringup from robotic_arm_bringup.',
    )
    ...
    panda_bringup = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([FindPackageShare('robotic_arm_motion'), 'launch', 'panda_bringup.launch.py'])
        ),
    ...
    return LaunchDescription(
        [
            ...
            panda_bringup,
            Node(
                package='robotic_arm_motion',
                executable='motion_05_workspace_reject',
                name='motion_05_workspace_reject',
                output='screen',
                parameters=[
                    {
