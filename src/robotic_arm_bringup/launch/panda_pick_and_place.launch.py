...
def _resolve_share_file(package_name, *relative_parts):
    package_share_path = get_package_share_directory(package_name)
    installed_path = os.path.join(package_share_path, *relative_parts)
...
def _resolve_moveit_config(file_name):
    return _resolve_share_file('robotic_arm_moveit_config', 'config', 'panda', file_name)
...
def generate_launch_description():
    ...
    description_share_path = get_package_share_directory('robotic_arm_description')
    sim_share_path = get_package_share_directory('robotic_arm_sim')
    control_share_path = get_package_share_directory('robotic_arm_control')
    xacro_file = _resolve_share_file(
        'robotic_arm_description', 'urdf', 'panda', 'panda.xacro.urdf'
    )
    controllers_file = os.path.join(control_share_path, 'config', 'panda', 'controllers_position.yaml')
    panda_srdf_file = _resolve_moveit_config('panda.srdf.xacro')
    panda_kinematics_file = _resolve_moveit_config('kinematics.yaml')
    panda_joint_limits_file = _resolve_share_file(
        'robotic_arm_control', 'config', 'panda', 'joint_limits.yaml'
    )

    resource_paths = [
        sim_share_path,
        os.path.join(sim_share_path, 'models'),
    ]
...
""".strip()
    ...
    moveit_controller_config = _load_yaml(
        _resolve_moveit_config('moveit_controller_manager.yaml')
    )
        ...
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value=PathJoinSubstitution(
                    [FindPackageShare('robotic_arm_moveit_config'), 'rviz', 'view.rviz']
                ),
            ...
            DeclareLaunchArgument(
                'world_file',
                default_value=PathJoinSubstitution(
                    [FindPackageShare('robotic_arm_sim'), 'worlds', 'simple_pick_and_place_collision.sdf']
                ),
